                ...
                <div class="col-lg-12">
                    <section class="panel">
                        ...
                        <div class="panel-body">
                            <form role="form" class="form-horizontal adminex-form" method="POST" action="{{ url('/admin/roles/'.$data['id'].'/assign') }}">
                                ...
                                <div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
                                    ...
                                    <div class="col-lg-10">
                                        <input type="text" placeholder="" id="name" name="name" disabled="disabled" class="form-control" value="{{ isset($data['name']) ? $data['name']:old('name') }}">
                                        @if ($errors->has('name'))
                                ...
                                <div>
                                    ...
                                    @foreach($permission_role as $p)
                                        @if($p['permission_id'] == $list['id'])
                                            checked="checked" 
                                        @endif 
                                    @endforeach
                                    value="{{ $list['id'] }}"/>{{ $list['label'] }}</br>
                                    @endforeach
                                </div>
